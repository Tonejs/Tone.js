	...
	describe("TransportTime", function(){
		...
		context("Constructor", function(){
			...
			it("can pass in a number in the constructor", function(){
				return Offline(function(Transport){
					...
					expect(time.eval()).to.equal(Transport.PPQ * 2);
					time.dispose();
				});
			});
			...
			it("can pass in a value and a type", function(){
				return Offline(function(Transport){
					expect(TransportTime(4, "m").eval()).to.equal(Transport.PPQ * 16);
				});
			});
			...
			it("with no arguments evaluates to the current ticks when the transport is started", function(){
				return Offline(function(Transport){
					...
					return Test.atTime(0.29, function(){
						expect(TransportTime().eval()).to.equal(Transport.ticks);
						Transport.stop();
					});
				}, 0.3);
			});
		...
		context("Quantizes values", function(){
			...
			it("can quantize values", function(){
				return Offline(function(Transport){
					expect(TransportTime("4n @ 2n").eval()).to.be.closeTo(Transport.PPQ, 0.01);
					expect(TransportTime("(1n + 4n) @ 4n").eval()).to.be.closeTo(Transport.PPQ * 5, 0.01);
					expect(TransportTime("4t").quantize("4n").eval()).to.be.closeTo(Transport.PPQ, 0.01);
				});
			});
			...
			it("can get the next subdivison when the transport is started", function(){
				return Offline(function(Transport){
					...
					return Test.atTime(0.59, function(){
						expect(TransportTime("@1m").eval()).to.be.closeTo(4 * Transport.PPQ, 0.01);
						expect(TransportTime("@(4n + 2n)").eval()).to.be.closeTo(Transport.PPQ * 3, 0.01);
						expect(TransportTime("@4n").eval()).to.be.closeTo(Transport.PPQ * 2, 0.01);
					});
				}, 0.6);
			});	
		});
		...
		context("Expressions", function(){
			...
			it("evaluates mixed expressions", function(){
				return Offline(function(Transport){
					expect(TransportTime("4n * 2").eval()).to.equal(Transport.PPQ * 2);
					expect(TransportTime("(4n * 2) / 4").eval()).to.equal(Transport.PPQ / 2);
					expect(TransportTime("0:2 / 2").eval()).to.equal(Transport.PPQ);
				});
			});
		});
			...
			it("can add the current time", function(){
				return Offline(function(Transport){
					...
					return Test.atTime(0.59, function(){
						...
						expect(TransportTime("2n").addNow().eval()).to.be.closeTo(Transport.PPQ * 2 + now, 0.01);
						expect(TransportTime("+2n").eval()).to.be.closeTo(Transport.PPQ * 2 + now, 0.01);
						Transport.stop();
					}, 0.6);
				});
			});
